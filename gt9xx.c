 ...
 *
 * Version: 2.1
 * Authors: andrew@goodix.com, meta@goodix.com, thug1337@gmail.com
 ...
 *                  By Meta, 2013/08/06
 *      V2.1:
 *          1. compatibilty with RK devices
 *          2. config & parameters set in board file rather than in header (to support many boards in single kernel tree)
 *                  By Bartosz J, 2014/06/13
 */
...
static unsigned GTP_INT_PORT = INVALID_GPIO;
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    msgs[0].buf   = &buf[0];
    msgs[0].scl_rate = GT9xx_I2C_RATE; //300 * 1000;    // for Rockchip, etc.
    
    msgs[1].flags = I2C_M_RD;
    msgs[1].addr  = client->addr;
    msgs[1].len   = len - GTP_ADDR_LENGTH;
    msgs[1].buf   = &buf[GTP_ADDR_LENGTH];
    msgs[1].scl_rate =  GT9xx_I2C_RATE;//300 * 1000;
...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
    ...
    msg.len   = len;
    msg.buf   = buf;
    msg.scl_rate = GT9xx_I2C_RATE;//300 * 1000;    // for Rockchip, etc
...
{
    s32 ret = 2;
	struct goodix_9xx_platform_data *pdata = client->dev.platform_data;
	if(pdata && pdata->send_config)
	{
		s32 retry = 0;
		struct goodix_ts_data *ts = i2c_get_clientdata(client);
...
{
    struct goodix_9xx_platform_data *pdata = ts->client->dev.platform_data;
    if(pdata && pdata->swap_xy)
	{
		GTP_SWAP(x, y);
	}

	if(pdata && pdata->mirrorxy)
	{
		x = ts->abs_x_max - x;
		y = ts->abs_y_max - y;
	}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_GPIO_OUTPUT(pdata->gpio_reset, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    GTP_GPIO_OUTPUT(pdata->gpio_irq, client->addr == 0x14);
    ...
    GTP_GPIO_AS_INPUT(pdata->gpio_reset);    // end select I2C slave addr
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	u8 *send_cfg_buf[] = {pdata->config_info};
	u8 cfg_info_len[] = {pdata->config_info_len};

    
    GTP_DEBUG_FUNC();
    		
    if(pdata->send_config && pdata->config_info_len)
    {

		GTP_DEBUG("Config Length: %d", pdata->config_info_len);
		...
		if(pdata->custom_config)
		{
			config[RESOLUTION_LOC]     = (u8)pdata->xmax;
			config[RESOLUTION_LOC + 1] = (u8)(pdata->xmax>>8);
			config[RESOLUTION_LOC + 2] = (u8)pdata->ymax;
			config[RESOLUTION_LOC + 3] = (u8)(pdata->ymax>>8);

			if (pdata->irq_edge == 0)  //RISING
			{
				config[TRIGGER_LOC] &= 0xfe;
			}
			else if (pdata->irq_edge == 1)  //FALLING
			{
...
{ // driver not send config
    ...
    if (ret < 0)
    {
        ...
        ts->abs_x_max = pdata->xmax;
        ts->abs_y_max = pdata->ymax;
        ts->int_trigger_type = pdata->irq_edge;
    }
    else
    {
		GTP_INFO("Read config from firmware: %dx%d : %d!", (config[RESOLUTION_LOC + 1] << 8) + config[RESOLUTION_LOC], (config[RESOLUTION_LOC + 3] << 8) + config[RESOLUTION_LOC + 2],
	...
	GTP_INFO("Final config is: %dx%d : %d!", ts->abs_x_max,ts->abs_y_max,ts->int_trigger_type);
    msleep(10);
    return 0;
}
...
Function:
    ...
    Unused, because init_platform_hw has taken control over gpios init (@bj)
Input:
    ts: private data.
Output:
    Executive outcomes.
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if (ret)
    {
        GTP_ERROR("Request (%d) IRQ failed!ERRNO:%d.", GTP_INT_PORT, ret);
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(!pdata)
	{
		GTP_ERROR("Platform data is not defined. Please define goodix_9xx_platform_data in your board config!");
		return -ENODEV;
	}
	...
	// TODO: Read device model from device
	GTP_INFO("GTP Device Model: %d", pdata->model);
	GTP_INFO("GTP SWAP XY: %d", pdata->swap_xy);
	GTP_INFO("GTP MIRROR XY: %d", pdata->mirrorxy);
		
	GTP_INT_PORT = pdata->gpio_irq;
	if( GTP_INT_PORT == INVALID_GPIO || GTP_INT_PORT == 0)
	{
		GTP_ERROR("You have to define INT GPIO to initialize GT9XX!!!");
		goto phail_board;
	}
    ...
    if (pdata->init_platform_hw) 
	{
		...
		{
			GTP_ERROR("Failed to initialize GT9XX (init_platform_hw)!!!");
			goto phail_board;
		}
	}
	else
	{
		GTP_ERROR("You have to define init_platform_hw to initialize GT9XX!!!");
		goto phail_board;
	}
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	{
		GTP_ERROR("You have to define exit_platform_hw to deinit GT9XX!!!");
		return -EFAULT;
	}
...
s32 gtp_i2c_read_no_rst(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    msgs[0].len   = GTP_ADDR_LENGTH;
    msgs[0].buf   = &buf[0];
    msgs[0].scl_rate = GT9xx_I2C_RATE;    // for Rockchip, etc.
